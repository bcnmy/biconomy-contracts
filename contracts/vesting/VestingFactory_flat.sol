...
contract TokenVesting {
     ...
     */
    function release(address token, uint256 amount) public {
        ...
        require(amount <= _releasableAmount(IERC20(token)), "Amount greater than releasable amount");
...
// File src/vesting/VestingFactory.sol
...
contract VestingFactory is Ownable{
  ...
  function revoke(address vestingContract) external onlyOwner{
    ...
    TokenVesting(vestingContract).revoke(bico, owner());
  ...
  function createVestingContract(address beneficiary, uint256 start, uint256 cliffDuration, uint256 duration, uint256 tokensVested) external onlyOwner{
        ...
        require(IERC20(bico).transfer(address(tokenVesting), tokensVested), "Transfer Failed");
